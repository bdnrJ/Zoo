...
const AddServiceType = ({closePopup}: props) => {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<ServiceType>({ resolver: zodResolver(schema) });

  const onSubmit = async (data: ServiceType) => {
    try {
      const response = await axiosClient.post('/service_types', {
        ...data,
        is_active: 0
    ...
    <div className="add_stype">
      ...
      <form className='__form' onSubmit={handleSubmit(onSubmit)}>
        ...
        <label htmlFor="description">
          <textarea placeholder='description' className={`${errors.description && "--error"}`} {...register('description', { required: true })} />
          {errors.description && <span className={`_inputError --big`}>{errors.description.message}</span>}
        </label>
        <label htmlFor="price_per_customer">
          <input type="number" placeholder='price per customer' className={`${errors.price_per_customer && "--error"}`} step="0.01" {...register('price_per_customer', { required: true, valueAsNumber: true })} />
        ...
        <label htmlFor="submitbtn" className="__orange-button-label">
            <button disabled={disabled} type="submit">Add Service Type</button>
        </label>
      </form>
    </div>
  )
